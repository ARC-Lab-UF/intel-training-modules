...
//   accum += x[i];
//
// This example demonstrates an alternative to using std::swap in
// each iteration, similar to what we show in accum_strategy4_1.cpp.
// The advantage of this approach is that it eliminates transfers
// to the host in between iterations.
//
// When running on the DevCloud, the execution time of this example
// for 1000000000 (1 billion) inputs was 1.96s, making it the fastest
...
int main(int argc, char* argv[]) { 
  ...
  try {
    ...
    // We can move the bufferes outside of the loop now because we aren't
    // swapping their pointers on the host. This avoids extra destruction in
    // each iteration, which triggers data transfers.
    cl::sycl::buffer<int, 1> x_buf {x_h.data(), cl::sycl::range<1>(vector_size) };
    cl::sycl::buffer<int, 1> y_buf {y_h.data(), cl::sycl::range<1>(ceil(vector_size / float(inputs_per_group))) };
    ...
    while(global_inputs_remaining > 1) {
	  ...
	  handler.parallel_for<class accum>(cl::sycl::nd_range<1>(num_groups * work_items_per_group, work_items_per_group), [=](cl::sycl::nd_item<1> item) {
	      ...
	      // we are now using "in" here instead of x_d because
	      // the input changes every iteration.
	      if (2*global_id + 1 == global_inputs_remaining) {
		x_local[local_id] = in[2*global_id];
	      }
	      else if (2*global_id + 1 < global_inputs_remaining) {
		x_local[local_id] = in[2*global_id] + in[2*global_id + 1];
	      }
		...
		item.barrier(cl::sycl::access::fence_space::local_space);
	      ...
	      // CHANGES FROM PREVIOUS VERSION
	      // Here we write to out instead of y_d because the output
	      // changes every iteration.
	      if (local_id == 0) {
		out[group_id] = x_local[0];
	      }
	    });
	});
